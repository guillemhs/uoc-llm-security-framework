pipeline {
    ...
    stages {
        ...
        stage('Prompt Injection Check') {
            steps {
                sh '. venv/bin/activate && ${PYTHON} audit/prompt_injection.py'
            }
        }
        ...
        stage('Toxicity Check') {
            steps {
                sh '. venv/bin/activate && ${PYTHON} audit/toxicity.py'
            }
        }
        ...
        stage('Bias Check') {
            steps {
                sh '. venv/bin/activate && ${PYTHON} audit/bias.py'
            }
        }
        ...
        stage('Data Leakage Check') {
            steps {
                sh '. venv/bin/activate && ${PYTHON} audit/data_leakage.py'
            }
        }
